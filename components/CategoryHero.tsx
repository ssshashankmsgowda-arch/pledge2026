...
const CategoryHero: React.FC<CategoryHeroProps> = ({ category, image, onAction, reversed }) => {
  return (
    <div className="max-w-7xl mx-auto px-6 py-6">
      <section 
        onClick={onAction}
        className={`cursor-pointer group relative flex flex-col md:flex-row items-stretch overflow-hidden bg-white rounded-[2.5rem] border border-stone-200 hover:border-red-200 transition-all duration-500 hover:shadow-2xl hover:shadow-red-50/50 ${reversed ? 'md:flex-row-reverse' : ''}`}
      >
        {/* Visual side */}
        ...
        <div className="w-full md:w-1/2 p-10 md:p-16 flex flex-col justify-center space-y-6">
          <div className="space-y-4">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-red-600 font-black text-[9px] uppercase tracking-[0.4em] outfit">Pathway 2025</span>
            </div>
          ...
          <div className="inline-flex items-center space-x-4 bg-stone-900 text-white px-8 py-4 rounded-2xl group-hover:bg-red-600 transition-all duration-300 w-fit">
